...
<a name="calculating-taxes"></a>
### Calculating Taxes

Thanks to [Stripe Tax](https://stripe.com/tax) it's possible to automatically calculate taxes for all invoices generated by Stripe. Cashier makes it easy for you to opt-in to this by enabling the behavior in the `boot` method of your application's `App\Providers\AppServiceProvider` class:
...
Now, any new subscription that you start or any one-off invoice you generate will automatically calculate the taxes for you. 

It's important to note that for this feature to work properly, your customer needs to have its details filled out in Stripe properly. Their name, address and Tax ID needs to be set for Stripe to be able to calculate the taxes properly. You may use the [syncing customer data](#syncing-customer-data-with-stripe) method and the [Tax ID](#tax-ids) methods to do that.

> {note} Since no invoice is generated for one-off payments, no tax is calculated for these type of payments. Stripe Tax is currently invite-only so make sure you [apply](https://stripe.com/tax#request-access) before enabling it in Cashier.
...
> {note} Instead of using Tax Rates as detailed below, you can [automatically calculate taxes using Stripe Tax](#calculating-taxes)
