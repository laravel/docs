...
> {note} Since no invoice is generated for one-off payments, no tax is calculated for these types of payments. In addition, Stripe Tax is currently "invite-only" during its beta period. You can request access to Stripe Tax via the [Stripe Tax website](https://stripe.com/tax#request-access).
