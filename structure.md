...
> [!NOTE]  
> Many of the classes in the `app` directory can be generated by Artisan via commands. To review the available commands, run the `php artisan list make` command in your terminal. Laravel can open them automatically for you if you provided your editor's path in the `app.editor` config or `APP_EDITOR` environment variable.
