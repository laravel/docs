...
The `routes` directory contains all of the route definitions for your application. By default, several route files are included with Laravel: `web.php`, and `console.php`.
...
Optionally, you may install additional route files for API (api.php) and broadcasting (channels.php), via the `install:api` and `install:broadcasting` Artisan commands.

The `api.php` contains routes that are intended to be stateless, so requests entering the application through these routes are intended to be authenticated [via tokens](/docs/{{version}}/sanctum) and will not have access to session state.
