...
In addition to inserting records into the database, the query builder can also update existing records using the `update` method. The `update` method, like the `insert` method, accepts an array of column and value pairs indicating the columns to be updated. The `update` method will only update columns that are designated `fillable` on the model (see more about [mass assignment](/docs/{{version}}/eloquent#mass-assignment)). The `update` method returns the number of affected rows. You may constrain the `update` query using `where` clauses:
