...
> **備註：** Laravel 沒有預設的資料夾來放置類別形式的視圖組件。你可以自由的把它們放在你想要的地方。舉例來說，你可以放在 `App\Http\Composers` 資料夾內。
...
> **備註：** 所有的視圖組件會被 [服務容器 \(service container\)](/docs/5.0/container) 解析，所以你需要在視圖組件的建構子型別限制你所需的任何相依參數。
