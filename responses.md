...
Sometimes you may wish to redirect the user to their previous location, for example, after a form submission that is invalid. You may do so by using the global `back` helper function. However, make sure the route calling the `back` function is using the `web` middleware group or has all of the session middleware applied:
...
#### Populating Redirect Parameters Via Eloquent Models
...
Redirecting to a new URL and [flashing data to the session](/docs/{{version}}/session#flash-data) are typically done at the same time. Typically, this is done after successfully performing an action, allowing you to flash a success message to the session.
...
After the user is redirected to a new page, you may display the flashed message from the [session](/docs/{{version}}/session). For example, using [Blade syntax](/docs/{{version}}/blade):
