...
在 `bootstrap/autoload.php` 檔案中，更新 `$compiledPath` 變數成：
...
別忘了將任何額外的 Composer 相依套件加入你的 5.0 應用程式內，包含第三方程式碼（例如 SDKs）。

部分 Laravel 專用套件也許不相容於剛釋出的 Laravel 5 版本，請向套件管理者確認該套件支援 Laravel 5 的版本。在 Composer 內加入任何額外的套件後，請執行 `composer update`。

### 命名空間

預設情況下，Laravel 4 沒有在應用程式的程式碼中使用命名空間，所以，舉例來說，所有的 Eloquent 模型和控制器都放在「全域」的命名空間下。為了更快速的整合，Laravel 5 也允許您可以將這些類別一樣保留在全域的命名空間。

### 設定檔

#### 整合環境變數

複製新的 `.env.example` 檔案到 `.env`，在 `5.0` 這相當於原本的 `.env.php`。設定所有該有的設定值，像是 `APP_ENV` 和 `APP_KEY` （您的加密金鑰）、資料庫認證和快取驅動與 session 驅動。
...
Laravel 5.0 不再使用 `app/config/{environmentName}/` 目錄結構來提供對應該環境的設定檔，取而代之的是，將環境對應的設定值移到 `.env`，然後使用 `env('key', 'default value')` 來存取在設定檔中的值，您可以在 `config/database.php` 檔案內看到相關範例。
...
下一步，將所有的控制器移到 `app/Http/Controllers` 目錄下。既然在本指南中我們不打算整合到完整的命名空間，請將 `app/Http/Controllers` 添加到 `composer.json` 的 `classmap`，接下來，您可以從 `app/Http/Controllers/Controller.php` 基底抽象類別中移除命名空間，並確認整合過來的控制器是繼承這個基底類別。
...
您不需要移動任何 Laravel 4.0 預設的過濾器，像是 `auth` 和 `csrf` 。他們已經內建其中，只是換作以中介層形式出現。那些在路由或控制器內有使用到舊有預設過濾器 (例如 `['before' => 'auth']`) 請修改使用新的中介層（ 例如 `['middleware' => 'auth'].` ）
...
將所有的命令類別從舊的 `app/commands` 目錄移到新的`app/Console/Commands` 目錄。接下來，把 `app/Console/Commands` 目錄添加到 `composer.json` 的`classmap` 中。
...
基於安全考量，Laravel 5.0 會轉譯所有輸出，不論您使用 `{{ }}` 或 `{{{ }}}` 標籤。您可以使用新的 `{!! !!}` 標籤來取消輸出轉譯。請務必**確定**輸出內容是安全的才使用 `{!! !!}` 標籤。
...
將 `4.2` 版 public 內的 assets 移到新應用程式內的 `public` 目錄內。並確認保留 `5.0` 版的 `index.php` 檔案。
...
如果您的程式注入 `Illuminate\Cache\CacheManager` 來取得非 Facade 版本的 Laravel 快取，請改用 `Illuminate\Contracts\Cache\Repository` 取代。
