...
When using the old style of factories in combination with the new factory classes, it's best to place your new factories in a separate directory like `database/new-factories`. Then update your `composer.json`'s `autoload` settings to the following:

    "autoload": {
        "psr-4": {
            "App\\": "app/",
            "Database\\NewFactories\\": "database/new-factories/",
            "Database\\Seeders\\": "database/seeders/"
        },
        "classmap": [
