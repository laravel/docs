...
- [The Dev Command](#the-dev-command)
    ...
    - [Listing Dev Processes](#listing-dev-processes)
...
The `dev` Artisan command starts all of the processes needed for local development in a single terminal window. By default, it runs the PHP development server, a queue worker, log tailing via [Pail](/docs/{{version}}/pail), and Vite asset compilation concurrently:
...
> [!NOTE]
> The `vite` process automatically detects your Node package manager (Bun, pnpm, Yarn, or npm) and uses the appropriate run command.
...
If you register a process with the same name as a default process, your process will replace the default. For example, to customize the server process to use a different port:
...
You may limit the `dev` command to only run specific processes using the `only` method. Similarly, you may exclude specific processes using the `except` method:
...
<a name="listing-dev-processes"></a>
### Listing Dev Processes
...
You may filter the list by name or command using the `--filter` option:

```shell
php artisan dev:list --filter=server
```

To output the list as JSON, use the `--json` option:

```shell
php artisan dev:list --json
```

The `dev:list` command also supports `--except-vendor` and `--only-vendor` options to filter processes by their registration source:

```shell
# Exclude processes registered by vendor packages...
php artisan dev:list --except-vendor

# Only show processes registered by vendor packages...
