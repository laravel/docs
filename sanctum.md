...
Once CSRF protection has been initialized, you should make a `POST` request to the typical Laravel `/login` route. This `/login` route may be implemented manually or using one of Laravel's [application starter kits](/docs/{{version}}/starter-kits).
