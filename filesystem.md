...
The `put` method may be used to store raw file contents on a disk. You may also pass a PHP `resource` to the `put` method, which will use Flysystem's underlying stream support. Please note that the file name will be relative to the selected disk, or to the default one if no disk is selected. Using streams is greatly recommended when dealing with large files:
