...
When using the `local` driver, note that all file operations are relative to the `root` option defined in your configuration file. By default, this value is set to the `storage/app` directory. Therefore, the following method would storage a file in `storage/app/file.txt':
