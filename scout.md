...
If you are using Scout with a queue, you need add the queue name `queue` to the worker, `php artisan queue:work` runs only the default queue if no name is specified:
