...
<a name="creating-index"></a>
### Creating Index

The `index` command may be used to create an index for a given model. The index name will be generated by `searchableAs()` method on the model.

```shell
php artisan scout:index "App\Models\Post"
```

If the model is soft deletable, Scout will automatically include support for filtering on soft deleted models on that index.

<a name="deleting-index"></a>
### Deleting Index

_Be careful, these commands are irreversible._

The `delete-index` command may be used to delete an index.
````shell
php artisan scout:delete-index "index_name"
````

The `delete-all-indexes` command may be used to delete all indexes.
