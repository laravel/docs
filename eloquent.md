...
<a name="uuid-and-ulid-primary-keys"></a>
#### UUID and ULID Primary Keys

Eloquent Models can use UUID as primary keys through the `Illuminate\Database\Eloquent\Concerns\HasUuid` trait. When used in your Model, Eloquent will automatically generate a sortable UUID v7-like as primary key before persisting the record into the database. The only requisite is to have an [UUID equivalent column](migrations.md#column-method-uuid) as primary key.
    ...
    $article = Article::create(['title => 'Traveling to Europe', 'body' => '...']);
    
    $article->id; // "8f8e8478-9035-4d23-b9a7-62f4d2612ce5"

The UUID generated by [`Str::orderedUuid()`](helpers.md#method-str-ordered-uuid) can be sorted lexicographically, which is similar to the [proposed UUID v7](https://datatracker.ietf.org/doc/html/draft-peabody-dispatch-new-uuid-format). You can still override both the UUID generation and the Model attributes these should be generated for.
    ...
    {
        return [$this->getKeyName(), $this->alternative_uuid];
    }

You may also use [ULID](https://github.com/ulid/spec) as primary key with the `Illuminate\Database\Eloquent\Concerns\HasUlid` trait. An ULID is a sortable 26-character string with the convenience of being UUID v7-like, thus UUID compatible. It requires a [ULID equivalent column](migrations.md#column-method-ulid) as primary key, and install the `symfony/ulid` library.
    ...
    $article = Article::create(['title => 'Traveling to Asia', 'body' => '...']);
