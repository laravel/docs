...
The UUID is generated by `Str::orderedUuid()`, which is similar to the [proposed UUID v7](https://datatracker.ietf.org/doc/html/draft-peabody-dispatch-new-uuid-format), which can be sorted lexicographically. You can still override both the UUID generation and which Model attributes these should be generated for.
...
You can also use [ULID](https://github.com/ulid/spec) as primary key with the `Illuminate\Database\Eloquent\Concerns\HasUlid` trait. An ULID is a sortable 26-character string with the convenience of being UUID v7-like. It requires a [ULID column](migrations.md#column-method-ulid) as primary key, and install the `symfony/ulid` library.
