...
The `apply` and `remove` methods both receive an `Illuminate\Database\Eloquent\Builder` query builder object and an instance of the model that the query is being built upon. The `apply` is responsible for adding any additional `where` clauses that the scope wishes to add and the `remove` is responsible for reversing the action taken by `apply`. In other words, `remove` should remove the `where` clause (or any other clause) that was added. So, for our `SoftDeletingScope`, the methods look something like this:
