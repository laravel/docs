...
Eloquent Models can be compatible with UUID as primary keys through the `Illuminate\Database\Eloquent\Concerns\HasUuid` trait. When used in your Model, Eloquent will automatically generate a sortable UUID v7-like for the model primary key before persisting the record into the database. The only requisite is to have an [UUID equivalent column](migrations.md#column-method-uuid) as primary key.
...
The UUID is generated by `Str::orderedUuid()`, which is similar to the [proposed UUID v7](https://datatracker.ietf.org/doc/html/draft-peabody-dispatch-new-uuid-format), making it sortable. You can still override both the UUID generation and which Model attributes these should be generated for.
...
You can also use [ULID](https://github.com/ulid/spec) as primary key with the `Illuminate\Database\Eloquent\Concerns\HasUlid` trait. An ULID is a 26-character string with the convenience of being UUID v7-like and sortable. It requires a [ULID column](migrations.md#column-method-ulid) as primary key, and the `symfony/ulid` library to generate them.

    composer require symfony/ulid
