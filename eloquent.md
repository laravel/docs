...
Eloquent Models can use UUID as primary keys through the `Illuminate\Database\Eloquent\Concerns\HasUuid` trait. When used in your Model, a sortable UUID v7-like as primary key will be generated before persisting the record into the database. The only requisite is to have an [UUID equivalent column](migrations.md#column-method-uuid) as primary key.
