...
- [Hashing](#hashing)
  ...
  - [Basic Usage](#basic-usage)
    ...
    - [Allowing to Rehash Password Using Different Algorithm](#allowing-to-rehash-password-using-different-algorithm)
...
<a name="allowing-to-rehash-password-using-different-algorithm"></a>

### Allowing to Rehash Password Using Different Algorithm

Sometimes, you need to change the hash driver on the fly, eg: from `bcrypt` to `argon2id` or vice versa. To allow the newly-verified user password to be rehashed with different hasher algorithm on user authentication attempt, you should change the `verify` option value on the `hashing` configuration file to `false`:
...
If the `verify` options set to `true`, the password can only be rehashed with the same algorithm if the work factor of current algorithm was changed.
