...
If you are experiencing Cross-Origin Resource Sharing (CORS) issues in the browser while fetching assets from the Vite dev server, you may need to grant your custom origin access to the dev server. Vite combined with the Laravel plugin allow the following origins without any additional configuration:
...
The easiest way to allow a custom origin for your project is ensure that your application's `APP_URL` environment variable matches the origin you are visiting in your browser. For example, if you visiting `https://my-app.laravel`, you should update your `.env` to match:

```env
APP_URL=https://my-app.laravel
```

If you need more fine-grained control over the origins, e.g., if you need to support multiple origins, you should fallback to [Vite's comprehensive and flexible built-in CORS server configuration](https://vite.dev/config/server-options.html#server-cors). As an example, you may specify multiple origins in the `server.cors.origin` configuration option in the project's `vite.config.js` file:
...
You may also include regex patterns, which can help if you would like to allow all origins for a given top-level domain, such as `*.laravel`:
