...
`handle` 方法也可以使用型別暗示相依，並且藉由 [服務容器](/docs/{{version}}/container) 機制自動進行依賴注入。例如：
...
想瞭解更多關於佇列命令的方法，請見[佇列文件](/docs/{{version}}/queues).
...
命令管線類別是透過 [IoC 容器](/docs/{{version}}/container) 來解析的，所以請自由的在建構式暗示所需的相依型別。
