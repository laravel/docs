...
For example, if you'd like to use `.test` instead of `.dev`, run `valet domain test` and Valet will start serving your projects at `*.test` automatically.

See [IANA documentation](https://www.iana.org/assignments/special-use-domain-names) for a list of safe TLDs to use without risk of clashing with commercially-registered domains.
