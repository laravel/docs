...
You can also drop a foreign key constraint. The default foreign key names follow the same convention as the other indexes created by the Schema builder. For example, to drop the foreign key we created above you use:

	$table->dropForeign('posts_user_id_foreign');

> **Note:** The referenced table must be created before the table with the foreign key.

> **MySQL:** The field referenced in the foreign key is very likely an auto increment and therefore automatically an unsigned integer. Please make sure to create the foreign key field with unsigned() as both fields have to be the exact same type and set the engine on both tables to InnoDB.
