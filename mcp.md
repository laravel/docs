...
Web servers are what you need 99% of the time and are accessible via HTTP POST requests, making them ideal for remote AI clients or web-based integrations. Register a web server using the `web()` method:
...
You can authenticate web MCP servers with middleware as you would for routes. This will require a user to authenticate before using any capability of the server.

<a name="oauth"></a>

## OAuth 2.1

The recommended way to protect your web-based MCP servers is with OAuth through [Laravel Passport](/docs/{{version}}/passport). 



#### Overview

Add the `Mcp::oauthRoutes()` helper to your `routes/ai.php` file to register the required OAuth2 discovery and client registration endpoints.
...
Start by following the installation and deployment steps for [Laravel Passport](/docs/{{version}}/passport#installation). You should have an `OAuthenticatable` model, new authentication guard, and passport keys before moving on.

```markdown
> [!TIP]
> Managing passport keys via [config & env](/docs/{{version}}/passport#loading-keys-from-the-environment) is much simpler.
```



To get going quickly you can publish the package provided authorization view:
...
Then tell Passport to use this view through your `AppServiceProvider`'s `boot` method:
...
```markdown
> [!NOTE]
> With this setup we're simply using OAuth as a translation layer to the underlying authenticatable model. We are ignoring many aspects of OAuth, such as scopes.
```



#### Using an existing Passport setup

Laravel MCP should work within your existing setup, but custom scopes aren't currently supported as OAuth is primarily used as a translation layer to the underlying authenticatable model.



The package, through `Mcp::oauthRoutes();` above adds, advertises, and uses an `mcp:use` scope only. 



#### Using an external OAuth authorization server
...
OAuth2.1 is the documented authentication mechanism in the Model Context Protocol specification, and is the most widely supported amongst MCP clients. For that reason we recommend using Passport where possible.



If you are already using [Sanctum](/docs/{{version}}/sanctum) then adding Passport will be tricky. In this instance we'd recommend using Sanctum without Passport, until you have a clear necessary MCP client that only supports OAuth.
...
If you'd like to protect your MCP server using [Sanctum](/docs/{{version}}/sanctum), simply add the Sanctum middleware to your server in `routes/ai.php`. Make sure MCP clients pass the usual `Authorization: Bearer x|token` header to be authenticated.
...
```php
public function handle(Request $request, Authenticatable $user)
{
