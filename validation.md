...
You may use the `Rule` class to fluently define the rule. In this class, we'll also specify the validation rules as an array instead of using the `|` character to delimit them:

    'state' => Rule::exists('states'),
...
If you would like to customize the query executed by the validation rule, for example, add an additional condition to exists query:
...
Out of the box the validation query would be something like:
    
    SELECT count(*) as aggregate
    FROM `users`
    WHERE `personal_email` = 'personal@email.email'
