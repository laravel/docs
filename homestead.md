...
Laravel provides a convenient way to [schedule Cron jobs](/docs/{{version}}/scheduling) by scheduling a single `schedule:run` Artisan command to be run every minute. The `schedule:run` will then examine your job scheduled defined in your `App\Console\Kernel` class to determine which jobs should be run.
