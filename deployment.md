...
The `optimize:clear` method may be used to remove all of the cache files generated by the `optimize` command, and clear the keys in the default cache driver:
...
> [!WARNING]  
> The `optimize:clear` command will also run the `cache:clear` command. The `cache:clear` command clears all keys in the default cache store configured in the cache config using `Cache::flush()`. You can read more about `Cache::flush()` in [the cache documentation](/docs/{{version}}/cache#removing-items-from-the-cache).
