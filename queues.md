...
An alternative to supervisor is Systemd. Systemd comes shipped with most linux flavours already. to configure you would need to create a new `service`. Examine output of `systemctl status` command to figure out where `*.service` files are located. Then create a new file of your own.
