...
The `queue:failed` command will list the job ID, connection, queue, class and failure time. The job ID may be used to retry the failed job. For instance, to retry a failed job that has an ID of `5`, issue the following command:
