...
If you want to avoid timeout errors it's be best to manually set a timeout for the lock with the same amount of seconds (or more) used to throttle the job by using the `block` method. This will make sure the lock is reserved for the given amount of seconds before it becomes available again:
