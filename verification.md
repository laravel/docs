...
If an unverified user attempts to access a route that has been assigned this middleware, they will automatically be redirected to the `verification.notice` [named route](/docs/{{version}}/routing#named-routes). Note that the `verified` middleware always needs to be combined with the `auth` middleware.
