...
By default, Laravel will use native Redis clustering, as the `options.cluster` configuration value is set to `redis`. Redis clustering is a great default option, as it gracefully handles failover.

Laravel also supports client-side sharding, which allows you to pool nodes and create a large amount of available RAM. While client-side sharding does not handle failover, it is better suited for transient cached data that is available from another primary data store.
