...
Laravel uses the `utf8mb4` character set by default, which includes support for storing "emojis" in the database. If you are running a version of MySQL older than the 5.7.7 release or MariaDB older than the 10.2.2 release, you could either decrease the index size at the expense of the string size, or decrease the string size at the expense of the index size.

##### Decreasing Index Size
You can decrease the index size as follows:

    $table->index([DB::raw('email(20)')]);
    
Or if it is a unique index:

    $table->unique([DB::raw('email(20)')]);
    
Considering this, you could change the migration of `password_resets`'s `up()` method to look like this:
...
##### Decreasing String Size
You can configure the default string length to be up to `191` for InnoDB. The reasoning behind it is that mb4, which is four bytes in length, would allow up to `191.25` chars: `(767 - 2) / 4 = 191.25`, where the 2 is for the stored field length and where [the default InnoDB limit is 767](https://dev.mysql.com/doc/refman/5.7/en/create-index.html)).

You may configure this by calling the `Schema::defaultStringLength` method within a server provider like `AppServiceProvider`:
