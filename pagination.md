...
If you are using Laravel's default Tailwind pagination views with Tailwind CSS v4 engine, you should ensure your application's main CSS file references Laravel's pagination views using the [`@source`](https://tailwindcss.com/docs/functions-and-directives#source-directive) directive, so that their Tailwind classes are not purged. Note that the path to the CSS file should be specified relative to the file, in our example, this file is `./resources/css/app.css`:
...
If you are using Laravel's default Tailwind pagination views with Tailwind CSS v3 engine, you should ensure your application's `tailwind.config.js` file's `content` key references Laravel's pagination views so that their Tailwind classes are not purged:
