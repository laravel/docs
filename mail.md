...
Laravel fires the `mailer.sending` event just before sending mail messages. Remember that this event is fired when the mail is *sent*, not when it is queued. You may register an event listener in your `EventServiceProvider`:
