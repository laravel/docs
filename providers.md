...
但我們所說的「啟動」指的是什麼？一般而言，我們指的是**註冊**事物，包括註冊服務容器繫結、事件監聽器、中介層，甚至路由。服務提供者是設定你的應用程式的中心所在。
...
所有的服務提供者都繼承了 `Illuminate\Support\ServiceProvider` 類別。你必須在此抽象類別的提供者上定義至少一個方法：`register`。在 `register` 方法中，你應該**只將事物繫結至[服務容器](/docs/{{version}}/container)之中**。你永遠不該試圖在 `register` 方法中註冊任何事件監聽器、路由或任何其他功能。
...
如同之前提到的，在 `register` 方法中，你應該只將事物繫結至[服務容器](/docs/{{version}}/container)中。你永遠不該試圖在 `register` 方法中註冊任何事件監聽器、路由或任何其他功能。否則的話，你可能會意外地使用到由尚未載入的服務提供者所提供的服務。
...
因此，若我們需要在我們的服務提供者中註冊一個視圖 composer 呢？這應該在 `boot` 方法中完成。**此方法會在所有其他的服務提供者被註冊後才被呼叫**，意味著你能存取框架中已註冊的所有其他服務：
...
若你的提供者**僅**於[服務容器](/docs/{{version}}/container)中註冊繫結，你可以選擇延緩其註冊，直到真正需要其中已註冊的繫結。延緩像這樣的提供者載入可增進你的應用程式的效能，因為這樣就毋須每個請求都從檔案系統將其載入。

要延緩提供者載入，將 `defer` 性質設定為 `true`，並定義一個 `provides` 方法。`provides` 方法會回傳提供者所註冊的服務容器繫結：
