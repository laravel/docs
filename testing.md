...
It is often useful to wrap every test case in a database transactions so that the data generated by one test does not interfere with subsequent tests. Laravel provides a convenient `DatabaseTransactions` trait that will automatically wrap each test in a transaction:
